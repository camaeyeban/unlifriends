	...
	if(fp != NULL){
		...
		if(detector == 1){
			while(fp != NULL && !feof(fp)){
				...
				fscanf(fp, "%s %s %s ", newnode1->name, newnode1->StudNum, newnode1->degree);
				fscanf(fp, "%d", &counter);
...
void save(node *h, node *t){	// saves the students' record to a file
	...
	while(temp1->next != NULL){
		...
		temp2 = temp1->friends->next;
		fprintf(fp, "%s %s %s ", temp1->name, temp1->StudNum, temp1->degree);	// writes students' record to the file
